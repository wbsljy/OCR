...
def add_native_good_rate_chart(ws, final_data: pd.DataFrame, anchor: str = "A2") -> None:
    """
...
def build_dashboard_export_bytes(
    ...
    if key == "沖壓" and process == "鍛壓" and not data.empty:
        ...
        agg = {c: "sum" for c in num_cols} | {"product_name": "first"}
        d = d.groupby("_day", as_index=False, sort=True).agg(agg)
        cn = {"_day": "生產日期", "input_total": "總投入數", "bad_total": "總不良數", "product_name": "品名"}
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as w:
        ...
        if final_data is not None:
            ...
            add_native_good_rate_chart(ws, final_data, anchor="A2")

        w.sheets["数据看板"]["A17"] = "前五项不良"
